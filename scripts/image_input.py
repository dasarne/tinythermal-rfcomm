...
def _render_svg_to_png_bytes(src_path: Path, width_px: int | None = None, height_px: int | None = None) -> bytes:
    errors = []
    ...
    if proc.returncode == 0 and proc.stdout:
        return proc.stdout
    errors.append(f"rsvg-convert: {proc.stderr.decode(errors='ignore').strip()}")
    ...
    if proc.returncode == 0 and proc.stdout:
        return proc.stdout
    errors.append(f"magick: {proc.stderr.decode(errors='ignore').strip()}")
...
def load_image_any(src_path: Path, svg_pixels_per_mm: float = 8.0) -> Image.Image:
    ...
    if suffix == ".svg":
        ...
        png_bytes = _render_svg_to_png_bytes(src_path, width_px=width_px, height_px=height_px)
